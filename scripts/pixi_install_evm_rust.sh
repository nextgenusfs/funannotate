...
# scripts/benchmark_predict_evm.sh) and gets copied to opt/evm-rust/src, which
...
EVM_SRC="${EVM_INSTALL_PREFIX}/src"
...
# Pinned to a specific commit for reproducibility. Override with
# EVM_RUST_COMMIT to bump deliberately; check
# https://github.com/hyphaltip/EVidenceModeler/commits/rust_optimize
# for the latest commit before bumping.
EVM_COMMIT="${EVM_RUST_COMMIT:-c41c3ed17b0f7cc2f06bffaf463eb55ae53f4714}"
EVM_SOURCE_DIR=""
EVM_CLONE_DIR=""

if [ -d "${EVM_LOCAL}/evm" ] && [ -f "${EVM_LOCAL}/evm/Cargo.toml" ]; then
    echo "[pixi_install_evm_rust] Using local EVidenceModeler checkout from ${EVM_LOCAL}..."
    EVM_SOURCE_DIR="${EVM_LOCAL}"
else
    echo "[pixi_install_evm_rust] Local EVidenceModeler not found or missing evm/, cloning from ${EVM_REPO} (commit ${EVM_COMMIT})..."
    ...
    if [ ! -f "${EVM_SOURCE_DIR}/evm/Cargo.toml" ]; then
        echo "[pixi_install_evm_rust] ERROR: evm/Cargo.toml not found in cloned ${EVM_REPO}@${EVM_COMMIT}" >&2
        return 1
    fi
fi

echo "[pixi_install_evm_rust] Building Rust EVidenceModeler from ${EVM_SOURCE_DIR}..."
...
for binary in "${EVM_BINARIES[@]}"; do
    if [ -x "${RELEASE_DIR}/${binary}" ]; then
        ...
        echo "[pixi_install_evm_rust] Installed ${binary}"
    else
        echo "[pixi_install_evm_rust] WARNING: ${binary} not found after build"
    fi
done
...
echo "[pixi_install_evm_rust] Installed EvmUtils to ${EVM_SRC}"
