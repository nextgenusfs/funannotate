...
import sys, os, multiprocessing, subprocess, csv, time, re, shutil
...
def tblastnFilter(input, query, cpus, output):
    ...
    subprocess.call(['tblastn', '-num_threads', str(cpus), '-db', 'genome', '-query', query, '-max_target_seqs', '1', '-db_soft_mask', '11', '-threshold', '999', '-max_intron_length', '3000', '-evalue', '1e-10', '-outfmt', '6', '-out', 'filter.tblastn.tab'], cwd = output, stdout = FNULL, stderr = FNULL)
...
def runExonerate(input):
    ...
    with open(exonerate_out, 'w') as output:
        subprocess.call(['exonerate', '--model', 'p2g', '--showvulgar', 'no', '--showalignment', 'no', '--showquerygff', 'no', '--showtargetgff', 'yes', '--percent', '80', '--ryo', ryo , query, scaffold], stdout = output, stderr = FNULL)
...
counter = 0
with open('exonerate.proteins.out', 'wb') as output:
...
#Finally write the EVM output from the combined exonerate output
perl = '/usr/bin/perl'
EVM = os.environ['EVM_HOME']
EVM_exonerate = os.path.join(EVM, 'EvmUtils', 'misc', 'exonerate_gff_to_alignment_gff3.pl')
with open('protein_alignments.gff3', 'w') as output:
    subprocess.call([perl, EVM_exonerate, 'exonerate.proteins.out'], stdout = output)

#finally clean-up your mess
shutil.rmtree(tmpdir)

print "Script finished: %i alignments generated" % counter
