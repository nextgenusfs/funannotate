...
def main(args):
    ...
    if args.maker_gff:
        ...
        if args.rna_bam:
            if not lib.checkannotations(hintsBAM):
                ...
                #filter intron hints
                cmd = [os.path.join(parentdir, 'aux_scripts', 'filterIntronsFindStrand.pl'), MaskGenome, bamjoinedhints, '--score']
        ...
        if not args.exonerate_proteins: #this is alignments variable name is confusing for historical reasons...
            if args.protein_evidence:
                ...
                p2g_cmd = [sys.executable, P2G, '-p', prot_temp, '-g', MaskGenome, '-o', 
                        Exonerate, '--maxintron', str(args.max_intronlen), '--cpus', str(args.cpus), 
    ...
    -------------------------------------------------------" % (len(longest10), ', '.join([str(x) for x in longest10])))
                ...
                if len(Contigsmissing) > 0:
                    ...
                    with open(GeneMark, 'w') as output:
                        with open(GeneMarkTemp, 'r') as input:
                            lines = input.read().replace("Augustus", "GeneMark")
                            output.write(lines)
                lib.log.info('{:,} predictions from GeneMark'.format(lib.countGFFgenes(GeneMark)))
        ...
        if RunBusco: 
            ...
            if not lib.checkannotations(busco_final):
                ...
                if runbuscocheck:
                    lib.log.info("Running BUSCO to find conserved gene models for training ab-initio predictors")
                ...
                lib.log.info('{:,} valid BUSCO predictions found, now formatting for EVM'.format(len(busco_complete)))
                ...
                lib.fix_busco_naming(busco_fulltable, busco_augustus_tmp, busco_augustus)
                ...
                if Exonerate and Transcripts:
                    evm_cmd = [sys.executable, EVM_script, os.path.join(args.out, 'logfiles', 'funannotate-EVM_busco.log'), 
                        ...
                        '--protein_alignments', os.path.abspath(busco_proteins), '--transcript_alignments', os.path.abspath(busco_transcripts), 
                ...
                if total < 1:
                    ...
                    lib.log.info('{0:,}'.format(total) + ' total gene models from EVM, now validating with BUSCO HMM search')
                ...
                #rename models  
                lib.fix_busco_naming(os.path.join(args.out, 'predict_misc', 'busco_proteins', 'run_'+aug_species, 'full_table_'+aug_species+'.tsv'), EVM_busco, busco_final)
        ...
        if args.augustus_gff:
            ...
            if not lib.checkannotations(aug_out):
                ...
                lib.log.info("Running Augustus gene prediction using {:} parameters".format(aug_species))
        ...
        if 'snap' in RunModes:
            if not lib.checkannotations(SnapPredictions):
                ...
                lib.log.info('Existing snap predictions found {:}'.format(SnapPredictions))
        ...
        if 'glimmerhmm' in RunModes:
            if not lib.checkannotations(GlimmerPredictions):
                if RunModes['glimmerhmm'] == 'pretrained' and StartWeights['glimmerhmm'] > 0 and os.path.isdir(os.path.join(LOCALPARAMETERS, 'glimmerhmm')):
                    ...
                    lib.runGlimmerHMMTrained(MaskGenome, os.path.join(LOCALPARAMETERS, 'glimmerhmm'), os.path.join(args.out, 'predict_misc'), GlimmerPredictions)
                else:
                ...
                lib.log.info('Existing GlimmerHMM predictions found: {:}'.format(GlimmerPredictions))
    ...
    if args.keep_evm and os.path.isfile(EVM_out):
        ...
        base_evm = [sys.executable, EVM_script, os.path.join(args.out, 'logfiles', 'funannotate-EVM.log'), 
            str(args.cpus), EVMFolder, '--genome', MaskGenome, '--gene_predictions', Predictions, '--weights', Weights]
        if args.repeats2evm:
