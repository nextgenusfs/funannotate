...
parser.add_argument('-i','--input', required=True, help='folder of protein fasta files')
parser.add_argument('-m','--method', default='pfam', choices=['pfam', 'cazy'], help='database to search')
parser.add_argument('-d','--db', required=True, help='location of HMM database')
parser.add_argument('-c','--cpus', default=1, type=int, help='location of HMM database')
...
splitProts = [os.path.join(args.input, f) for f in os.listdir(args.input) if os.path.isfile(os.path.join(args.input, f))]
if args.method == 'pfam':
