...
open( FILE, ">$fasta_outfile" )
  and warn "Will write contigs to file '$fasta_outfile' and AGP to STDOUT\n"
...
while ( my $seq_obj = $inseq->next_seq ) {
    ...
    if (   $supercontig_id =~ m/NODE_(\d+)_length_\d+_cov_\d+/
        or $supercontig_id =~ m/^(\d+)$/ )
    {
