...
def runtbl2asn_parallel(folder, template, discrepency, organism, isolate, strain, parameters, version, cpus):
    ...
    cmd = ['tbl2asn', '-y', '"Annotated using '+fun_version+'"', '-N', str(version), 
    	   '-t', template, '-M', 'n', '-j', '"'+meta+'"', '-V', 'b', '-c', 'fx', '-T', '-a', 'r10u']
...
parser.add_argument('-i','--input', required=True, help='Genome in TLB format')
...
parser.add_argument('-d', '--discrep', required=True, help='discprepency report')
parser.add_argument('-t','--tbl2asn', default='-l paired-ends', help='Parameters for tbl2asn, linkage and gap info')
parser.add_argument('-v','--version', default=1, type=int, help='Genome version')
...
#now run in parallel
runtbl2asn_parallel(args.out, args.sbt, args.discrep, args.species, args.isolate, args.strain, args.tbl2asn, args.version, args.cpus)
