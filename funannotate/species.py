...
def showAll(dir):
    Table = [['Species', 'Augustus', 'GeneMark', 'Snap', 'GlimmerHMM', 'CodingQuarry', 'Date']]
    for f in os.listdir(dir):
        ff = os.path.join(dir, f)
        if os.path.isdir(ff) and lib.checkannotations(os.path.join(ff, 'info.json')):
            with open(os.path.join(ff, 'info.json')) as infile:
...
def main(args):
    ...
    parser.add_argument('-a', '--add', '--add-parameters', dest='add', help='Parameter JSON file to add to database')
    parser.add_argument('-p', '--parameters', dest='parameters', help='Parameter JSON file to add to database')
    ...
    if args.parameters: #just pretty-print JSON file
        ...
        if not os.access(os.path.join(FUNDB, 'trained_species'), os.W_OK | os.X_OK):
            print('ERROR: you do not have permissions to write to {:}'.format(os.path.join(FUNDB, 'trained_species')))
            sys.exit(1)
        if args.species in SpFound:
            print('ERROR: {:} is already in database, choose a different name or delete existing to continue'.format(args.species))
            sys.exit(1)
        print('Adding {:} to Database'.format(args.species))
        newLoc = os.path.abspath(os.path.join(FUNDB, 'trained_species', args.species))
        ...
        for x in data:
            if not 'path' in data[x][0]:
                continue
            newPath = os.path.join(newLoc, os.path.basename(data[x][0]['path']))
        ...
        if args.species in SpFound:
            ...
            print('{:} not found in Funannotate trained species folder'.format(args.species))
        ...
        print(lib.colour(' To print the parameters details from a species in the database:', 'none'))
        print(lib.colour('   funannotate species -s aspergillus_fumigatus', 'dim'))
        print(lib.colour(' To add a new species to database:', 'none'))
        print(lib.colour('   funannotate species -s new_species_name -a new_species_name.parameters.json\n', 'dim'))
