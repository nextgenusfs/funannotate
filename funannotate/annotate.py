...
def MEROPSBlast(input, cpus, evalue, tmpdir, output, diamond=True):
    ...
    if diamond:
        ...
        cmd = ['diamond', 'blastp', '--sensitive', '--query', input, '--threads', str(cpus), 
               '--out', blast_tmp, '--db', blastdb, '--evalue', str(evalue), '--max-target-seqs', '1', 
               '--outfmt', '5']
    else:
...
def SwissProtBlast(input, cpus, evalue, tmpdir, GeneDict, diamond=True):
    ...
    if diamond:
        ...
        cmd = ['diamond', 'blastp', '--sensitive', '--query', input, '--threads', str(cpus), 
               '--out', blast_tmp, '--db', blastdb, '--evalue', str(evalue), '--max-target-seqs', 
               '1', '--outfmt', '5']
    else:
...
def main(args):
    ...
    parser.add_argument('-t','--tbl2asn', default='-l paired-ends', help='Custom parameters for tbl2asn, example: linkage and gap info')
    ...
    if not lib.checkannotations(phobius_out):
        if lib.which('phobius.pl'):
            if not lib.checkannotations(phobius_out):
                ...
                subprocess.call([os.path.join(parentdir, 'aux_scripts', 'phobius-multiproc.py'), '-i', Proteins, '-o', phobius_out, '-l', phobiusLog])        
        else:
    ...
    cmd = [sys.executable, os.path.join(parentdir, 'aux_scripts', 'tbl2asn_parallel.py'), 
           '-i', TBLOUT, '-f', os.path.join(outputdir, 'annotate_misc', 'tbl2asn', 'genome.fsa'), 
           '-o', os.path.join(outputdir, 'annotate_misc', 'tbl2asn'), '--sbt', SBT, '-d', discrep, 
