...
def runTrimmomaticPE(left, right, cpus=1):
    ...
    cmd = [
        ...
        right_single,
        "ILLUMINACLIP:"
        + os.path.join(parentdir, "config", "TruSeq3-PE.fa")
        + ":2:30:10",
        "SLIDINGWINDOW:4:5",
        "LEADING:5",
        "TRAILING:5",
...
def runTrimmomaticSE(reads, cpus=1):
    ...
    cmd = [
        ...
        output,
        "ILLUMINACLIP:"
        + os.path.join(parentdir, "config", "TruSeq3-SE.fa")
        + ":2:30:10",
        "SLIDINGWINDOW:4:5",
        "LEADING:5",
        "TRAILING:5",
...
def runPASA(
    ...
    if configFile:
        ...
        with open(alignConfig, "w") as config1:
            with open(
                os.path.join(PASA, "pasa_conf", "pasa.alignAssembly.Template.txt"), "r"
            ) as template1:
                for line in template1:
                    if "<__DATABASE__>" in line:
                        ...
                        line = line.replace(
                            "<__MIN_PERCENT_ALIGNED__>", str(min_pct_aligned)
                        )
    ...
    with open(annotConfig, "w") as config2:
        with open(
            os.path.join(PASA, "pasa_conf", "pasa.annotationCompare.Template.txt"), "r"
        ) as template2:
    ...
    lib.runSubprocess(
        cmd, folder, lib.log, capture_output=pasaLOG1, capture_error="STDOUT"
    )
    round1GFF = None
    ...
    lib.runSubprocess(
        cmd, folder, lib.log, capture_output=pasaLOG2, capture_error="STDOUT"
    )
    round2GFF = None
    for file in os.listdir(folder):
        if not file.endswith(".gff3"):
...
def runSeqClean(input, folder, cpus=1):
    ...
    if os.path.isfile(input + ".clean"):
        lib.log.info(
            "Existing SeqClean output found: {:}".format(
                os.path.join(folder, input + ".clean")
            )
        )
    else:
        cmd = [
            os.path.join(PASA, "bin", "seqclean"),
...
def mapTranscripts(
    genome, longTuple, assembled, tmpdir, trinityBAM, allBAM, cpus=1, max_intronlen=3000
):
    """
    ...
    if not all(v is None for v in longTuple):
        ...
        if longTuple[0]:  # run iso-seq method
            ...
            if lib.checkannotations(assembled):
                unmapped_count = longReadFilter(isoMap, assembled, isoSeqs, cpus=cpus)
                lib.log.debug(
                    "{:,} IsoSeq reads not found in Trinity".format(unmapped_count)
                )
            else:
                isoSeqs = isoMap
        ...
        if longTuple[1]:  # run nano cDNA
            ...
            if lib.checkannotations(assembled):
                unmapped_count = longReadFilter(
                    nano_cDNA_Map, assembled, nano_cdnaSeqs, cpus=cpus
                )
                lib.log.debug(
                    "{:,} ONT cDNA reads not found in Trinity".format(unmapped_count)
                )
            else:
                nano_cdnaSeqs = nano_cDNA_Map
            lib.nanopore_cDNA_minimap2(
                nano_cdnaSeqs, genome, cpus, max_intronlen, nano_cdnaBAM
            )
        if longTuple[2]:  # run nano mRNA
            ...
            if lib.checkannotations(assembled):
                unmapped_count = longReadFilter(
                    nano_mrna_Map, assembled, nano_mrnaSeqs, cpus=cpus
                )
                lib.log.debug(
                    "{:,} ONT mRNA reads not found in Trinity".format(unmapped_count)
                )
            else:
                nano_mrnaSeqs = nano_mrna_Map
            lib.nanopore_mRNA_minimap2(
                nano_mrnaSeqs, genome, cpus, max_intronlen, nano_mrnaBAM
            )
        ...
        if len(mappedSeqs) > 0:
            ...
            lib.log.info(
                "Adding {:,} unique long-reads to Trinity assemblies".format(
    ...
    if lib.checkannotations(assembled):  # Trinity transcripts
        ...
        lib.minimap2Align(trinityCombinedClean, genome, cpus, max_intronlen, trinityBAM)
    else:
...
def runKallisto(input, fasta, readTuple, stranded, cpus, output):
    ...
    if len(ignore) > 0:
        lib.log.debug(
            "Ignoring %i incomplete PASA models: %s" % (len(ignore), ",".join(ignore))
        )
...
def getBestModels(input, fasta, abundances, alt_transcripts, outfile):
    ...
    if float(alt_transcripts) == 0:
        ...
        lib.log.info(
            "Parsing Kallisto results. Keeping alt-splicing transcripts if expressed at least {0:.1f}% of highest transcript per locus.".format(
                float(alt_transcripts) * 100
            )
        )
    else:
    ...
    lib.log.info(
        "Wrote {:,} transcripts derived from {:,} protein coding loci.".format(
            len(extractList), len(bestModels)
        )
    )


def GFF2tblCombinedNEW(
    evm,
    genome,
    trnascan,
    prefix,
    ...
    for k, v in list(sortedGenes.items()):
        ...
        if alt_transcripts == "1":
            ...
            if loc in gene_inter[v["contig"]]:
                for hit in list(gene_inter[v["contig"]].find(loc)):
                    if (
                        hit[3] != k and hit[2] == v["strand"]
                    ):  # same strand but diff gene
        ...
        if (
            k.startswith("novel_gene")
            or k.startswith("temp_gene")
            or k.startswith("split_gene")
        ):
            genenumber += 1
    ...
    lib.log.info(
        "Writing {:,} loci to TBL format: dropped {:,} overlapping, {:,} too short, and {:,} frameshift gene models".format(
...
def main(args):
    ...
    parser = argparse.ArgumentParser(
        prog="funannotate-update.py",
        usage="%(prog)s [options] -i genome.gbk -l left.fq.gz -r right.fg.gz",
    ...
    parser.add_argument(
        "-i", "--input", help="Genome in GBK format or funannotate folder"
    )
    ...
    parser.add_argument(
        "-c", "--coverage", default=50, type=int, help="Depth to normalize reads to"
    )
    parser.add_argument(
        "-m",
        "--min_coverage",
    ...
    parser.add_argument(
        "--pasa_min_pct_aligned", default="90", help="PASA --MIN_PERCENT_ALIGNED"
    )
    parser.add_argument(
        "--pasa_min_avg_per_id", default="95", help="PASA --MIN_AVG_PER_ID"
    )
    parser.add_argument(
        "--pasa_num_bp_splice",
        default="3",
    ...
    parser.add_argument(
        ...
        help="PASA SQL database to use",
    )
    parser.add_argument(
        "--memory", default="50G", help="RAM to use for Jellyfish/Trinity"
    )
    parser.add_argument(
        "--no_normalize_reads", action="store_true", help="skip normalization"
    )
    parser.add_argument(
        "--no_trimmomatic",
        action="store_true",
    ...
    parser.add_argument(
        "--name", help="Shortname for genes, perhaps assigned by NCBI, eg. VC83_"
    )
    parser.add_argument(
        "--max_intronlen", default=3000, help="Maximum intron length for gene models"
    )
    parser.add_argument(
        "--min_protlen",
        default=50,
    ...
    parser.add_argument(
        "--stranded",
        default="no",
        choices=["RF", "FR", "F", "R", "no"],
        help="RNA seq strandedness",
    )
    ...
    parser.add_argument(
        "--TRINITYHOME", help="Path to Trinity config directory, $TRINITYHOME"
    )
    parser.add_argument(
    ...
    if not os.path.isdir(args.out):
        ...
        dirs = [
            os.path.join(args.out, "update_misc"),
            os.path.join(args.out, "logfiles"),
            os.path.join(args.out, "update_results"),
        ]
        for d in dirs:
    ...
    if os.path.isfile(
        os.path.join(PASA, "Launch_PASA_pipeline.pl")
    ):  # then v2.3.0 or newer
        LAUNCHPASA = os.path.join(PASA, "Launch_PASA_pipeline.pl")
        PASAVERSION = "2.3.0"
    elif os.path.isfile(
        os.path.join(PASA, "scripts", "Launch_PASA_pipeline.pl")
    ):  # older version
        ...
        if os.path.isdir(args.input):
            ...
            if os.path.isdir(os.path.join(args.input, "predict_results")):
                for file in os.listdir(os.path.join(args.input, "predict_results")):
                    ...
                    if file.endswith(".stats.json"):
                        existingStats = os.path.join(
                            args.input, "predict_results", file
                        )
            ...
            if os.path.isdir(os.path.join(args.input, "training")):
                ...
                if lib.checkannotations(
                    os.path.join(inputDir, "trimmomatic", "trimmed_left.fastq.gz")
                ):
                    trim_left = os.path.join(
                        inputDir, "trimmomatic", "trimmed_left.fastq.gz"
                    )
                if lib.checkannotations(
                    os.path.join(inputDir, "trimmomatic", "trimmed_right.fastq.gz")
                ):
                    trim_right = os.path.join(
                        inputDir, "trimmomatic", "trimmed_right.fastq.gz"
                    )
                if lib.checkannotations(
                    os.path.join(inputDir, "trimmomatic", "trimmed_single.fastq.gz")
                ):
                ...
                if lib.checkannotations(
                    os.path.join(inputDir, "funannotate_train.stringtie.gtf")
                ):
                    stringtieGTF = os.path.join(
                        inputDir, "funannotate_train.stringtie.gtf"
                    )
                if lib.checkannotations(
                    os.path.join(inputDir, "funannotate_long-reads.fasta")
                ):
                    longReadClean = os.path.join(
                        inputDir, "funannotate_long-reads.fasta"
                    )
                if lib.checkannotations(
                    os.path.join(inputDir, "funannotate_train.trinity-GG.fasta")
                ):
                    trinity_results = os.path.join(
                        inputDir, "funannotate_train.trinity-GG.fasta"
                    )
                if lib.checkannotations(
                    os.path.join(inputDir, "funannotate_train.coordSorted.bam")
                ):
                ...
                if args.pasa_config:
                    ...
                    os.path.join(inputDir, "pasa", "alignAssembly.txt")
                ):
            ...
            look4files_names = [
                ...
                "Forward reads",
                "Reverse reads",
                "Forward Q-trimmed reads",
                "Reverse Q-trimmed reads",
                "Single Q-trimmed reads",
                "Forward normalized reads",
                "Reverse normalized reads",
                "Single normalized reads",
                "Trinity results",
                "Long-read results",
                "PASA config file",
                "BAM alignments",
            ...
            if len(files_used) > 0:
                lib.log.info(
                    "Found relevant files in %s, will re-use them:\n%s"
                    % (inputDir, "\n".join(files_used))
                )
        else:
            GBK = args.input
    ...
    lib.log.info(
        "Previous annotation consists of: {:,} protein coding gene models and {:,} non-coding gene models".format(
    ...
    if not all_reads:
        if not lib.checkannotations(os.path.join(tmpdir, "single.fq.gz")):
            if args.single:
                ...
                if s_reads.endswith(".fq"):
                    ...
                    s_reads = s_reads + ".gz"
                if not lib.checkannotations(os.path.join(tmpdir, "single.fq.gz")):
                    if os.path.dirname(os.path.abspath(tmpdir)) != os.path.dirname(
                        os.path.abspath(s_reads)
                    ):
                        lib.SafeRemove(os.path.join(tmpdir, "single.fq.gz"))
                        os.symlink(
                            os.path.realpath(s_reads),
        ...
        ) or not lib.checkannotations(os.path.join(tmpdir, "right.fq.gz")):
    ...
    if not trim_reads:
        if (
            ...
            if not os.path.isfile(
                os.path.join(tmpdir, "trimmomatic", "trimmed_left.fastq.gz")
            ) or not os.path.isfile(
                os.path.join(tmpdir, "trimmomatic", "trimmed_right.fastq.gz")
            ):
                ...
                trim_left, trim_right = os.path.join(
                    tmpdir, "trimmomatic", "trimmed_left.fastq.gz"
                ), os.path.join(tmpdir, "trimmomatic", "trimmed_right.fastq.gz")
            if (
                not os.path.isfile(
                    os.path.join(tmpdir, "trimmomatic", "trimmed_single.fastq.gz")
                )
                and s_reads
            ):
                if all_reads[2]:
    ...
    if (
        not trinity_results
        and all(v is None for v in norm_reads)
        and all(v is None for v in long_reads)
    ):
        ...
        if not all(v is None for v in long_reads):
            ...
            if not all(v is None for v in long_reads):
                if not lib.checkannotations(longReadFA):
                    long_readsFA, long_clean = long2fasta(
                        long_reads,
                        args.cpus,
                        tmpdir,
        ...
        if not lib.checkannotations(trinity_transcripts):
            if args.trinity:
                ...
                if not all(v is None for v in norm_reads):
                    ...
                    cmd = [
                        sys.executable,
                        os.path.join(parentdir, "aux_scripts", "trinity.py"),
                        "-f",
                        fastaout,
                        "-o",
                        trinity_transcripts,
                        ...
                        args.memory,
                        "--logfile",
                        os.path.join(args.out, "logfiles", "funannotate-trinity.log"),
                    ]
    ...
    if not stringtieGTF:
        ...
        if not lib.checkannotations(stringtieGTF):
            if lib.which("stringtie") and lib.checkannotations(shortBAM):
                ...
                cmd = cmd + [shortBAM]
                lib.runSubprocess(
                    cmd, ".", lib.log, capture_output=stringtieGTF, only_failed=True
                )

    # run SeqClean to clip polyA tails and remove low quality seqs.
    cleanTranscripts = trinity_transcripts + ".clean"
    if not lib.checkannotations(cleanTranscripts) and lib.checkannotations(
        trinity_transcripts
    ):
    ...
    if not lib.checkannotations(allBAM):
        trinity_transcripts, cleanTranscripts = mapTranscripts(
            ...
            allBAM,
            cpus=args.cpus,
            max_intronlen=args.max_intronlen,
        )
    else:
        if lib.checkannotations(trinityBAM):
            lib.log.info(
                "Existing BAM alignments found: {:}, {:}".format(trinityBAM, allBAM)
            )
        else:
    ...
    if all(v is None for v in kallistoreads) and lib.checkannotations(longReadClean):
        ...
        if not lib.checkannotations(minimapBAM):
            lib.log.debug(
                "{} | {}".format(" ".join(minimap2_cmd), " ".join(samtools_cmd))
            )
    ...
    getBestModels(
        PASA_gff, fastaout, KallistoAbundance, args.alt_transcripts, BestModelGFF
    )
    ...
    if WGS_accession:
        os.rename(
            os.path.join(tmpdir, "tbl2asn", "genome.tbl"),
            os.path.join(tmpdir, "tbl2asn", "genome.tbl.bak"),
        )
        p2g = {}
        if args.p2g:  # load into dictionary
            shutil.copyfile(
                args.p2g, os.path.join(args.out, "update_results", "ncbi.p2g")
            )
    ...
    discrep = os.path.join(
        args.out, "update_results", organism_name + ".discrepency.report.txt"
    )
    ...
    final_stats = os.path.join(
        args.out, "update_results", organism_name + ".stats.json"
    )
    ...
    with open(
        os.path.join(args.out, "update_results", "WGS_accession.txt"), "w"
    ) as out:
        out.write("%s\n" % WGS_accession)
    ...
    Changes = os.path.join(
        args.out, "update_results", organism_name + ".pasa-reannotation.changes.txt"
    )
    if args.gff and args.fasta:
