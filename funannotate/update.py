...
def runTrimmomaticPE(left, right, cpus=1):
    ...
    cmd = ['java', '-jar', os.path.join(TRIMMOMATIC_DIR, 'trimmomatic.jar'), 'PE', '-threads', str(cpus), '-phred33', 
            ...
            'ILLUMINACLIP:'+os.path.join(TRIMMOMATIC_DIR,'adapters','TruSeq3-PE.fa')+':2:30:10', 
...
def runTrimmomaticSE(reads, cpus=1):
    ...
    cmd = ['java', '-jar', os.path.join(TRIMMOMATIC_DIR, 'trimmomatic.jar'), 'SE', '-threads', str(cpus), '-phred33', 
            reads, output, 'ILLUMINACLIP:'+os.path.join(TRIMMOMATIC_DIR,'adapters','TruSeq3-SE.fa')+':2:30:10', 
            'SLIDINGWINDOW:4:5', 'LEADING:5', 'TRAILING:5', 'MINLEN:25']
    lib.runSubprocess(cmd, '.', lib.log)
...
def runNormalization(readTuple, memory, min_coverage=5, coverage=50, cpus=1, stranded='no'):
    ...
    if stranded != 'no':
        cmd = [os.path.join(TRINITY, 'util', 'insilico_read_normalization.pl'), '--PARALLEL_STATS', 
               '--JM', memory, '--min_cov', str(min_coverage), '--max_cov', str(coverage), 
...
def mapTranscripts(genome, longTuple, assembled, tmpdir, trinityBAM, allBAM, cpus=1, max_intronlen=3000):
    ...
    if not all(v is None for v in longTuple): #tuple is (iso-seq, nanopore_cDNA, nanopore_mRNA)
        ...
        if longTuple[0]: #run iso-seq method
            lib.iso_seq_minimap2(longTuple[0], genome, cpus, max_intronlen, isoBAM)
            bam2fasta(isoBAM, isoSeqs, cpus=cpus)
        if longTuple[1]: #run nano cDNA
           lib.nanopore_cDNA_minimap2(longTuple[1], genome, cpus, max_intronlen, nano_cdnaBAM)
           bam2fasta(nano_cdnaBAM, nano_cdnaSeqs, cpus=cpus)
        if longTuple[2]: #run nano mRNA
            lib.nanopore_mRNA_minimap2(longTuple[2], genome, cpus, max_intronlen, nano_mrnaBAM)
    ...
    if lib.checkannotations(assembled): #Trinity transcripts
        ...
        if lib.checkannotations(mappedLong):
            ...
            minimap_cmd = ['minimap2', '-ax', 'map-ont', '-t', str(cpus), '--secondary=no', assembled, mappedLong]
            cmd = [os.path.join(parentdir, 'aux_scripts', 'sam2bam.sh'), " ".join(minimap_cmd), str(cpus // 2), crosscheckBAM]
...
def GFF2tblCombinedNEW(evm, genome, trnascan, prefix, genenumber, justify, SeqCenter, SeqRefNum, tblout, alt_transcripts='1'):
    ...
    lib.dicts2tbl(renamedGenes, scaff2genes, scaffLen, SeqCenter, SeqRefNum, skipList, tblout)
...
def main(args):
        ...
        if os.path.isdir(args.input):
            ...
            if os.path.isdir(os.path.join(args.input, 'training')): #then funannotate train has been run, try to get reads, trinity, PASA
                ...
                if lib.checkannotations(os.path.join(inputDir, 'funannotate_train.stringtie.gtf')):
                    stringtieGTF = os.path.join(inputDir, 'funannotate_train.stringtie.gtf')
        ...
        if args.no_trimmomatic or args.trinity or left_norm or single_norm or args.left_norm or args.single_norm:
            ...
            if not os.path.isfile(os.path.join(tmpdir, 'trimmomatic', 'trimmed_left.fastq.gz')) or not os.path.isfile(os.path.join(tmpdir, 'trimmomatic', 'trimmed_right.fastq.gz')):
                if all_reads[0] and all_reads[1]:
                    trim_left, trim_right = runTrimmomaticPE(l_reads, r_reads, cpus=args.cpus)
                else:
    ...
    if not lib.checkannotations(allBAM):
        trinity_transcripts, cleanTranscripts = mapTranscripts(fastaout, long_clean, cleanTranscripts, tmpdir, trinityBAM, allBAM, cpus=args.cpus, max_intronlen=args.max_intronlen)
    else:
    ...
    if all(v is None for v in kallistoreads) and lib.checkannotations(longReadClean):
        ...
        cmd = [os.path.join(parentdir, 'aux_scripts', 'sam2bam.sh'), " ".join(minimap_cmd), str(args.cpus // 2), minimapBAM]
        ...
        if not lib.checkannotations(KallistoAbundance):
            runKallisto(PASA_gff, fastaout, kallistoreads, args.stranded, args.cpus, KallistoAbundance)
        else:
    ...
    if not lib.checkannotations(TBLFile):
        ...
        os.makedirs(gagdir)
        GFF2tblCombinedNEW(BestModelGFF, fastaout, cleanTRNA, locustag, genenumber, justify, args.SeqCenter, args.SeqAccession, TBLFile, alt_transcripts=args.alt_transcripts)
