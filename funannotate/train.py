...
def getBestModel(input, fasta, abundances, outfile, pasa_alignment_overlap=30):
    ...
    for scaffold in inter_gene:
        for x in inter_gene[scaffold]:
            ...
            for y in hits:
                percentOverlap = pOverlap(loc,[y[0],y[1]])
                if percentOverlap >= (float(pasa_alignment_overlap) / 100): #overlap more than args.pasa_alignment_overlap
...
def main(args):
	...
	parser.add_argument('--no_trimmomatic', '--no-trimmomatic', dest='no_trimmomatic', action='store_true', help='skip quality trimming via trimmomatic')
	...
	if not lib.checkannotations(allBAM):
		trinity_transcripts, cleanTranscripts = mapTranscripts(genome, long_clean, cleanTranscripts, tmpdir, trinityBAM, allBAM, cpus=args.cpus, max_intronlen=args.max_intronlen)
	else:
	...
	getBestModel(PASA_tmp, genome, KallistoAbundance, PASA_gff, pasa_alignment_overlap=args.pasa_alignment_overlap)
