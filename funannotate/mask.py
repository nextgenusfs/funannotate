...
def RepeatModelMask(input, cpus, tmpdir, output, repeatlib, debug):
    ...
    if not os.path.isfile(library):
        lib.log.info("Soft-masking: running RepeatMasker with default library (RepeatModeler found 0 models)")
        with open(debug, 'a') as debug_log:
...
def RepeatMaskSpecies(input, species, cpus, tmpdir, output, debug):
    ...
    outdir = os.path.join(tmpdir, 'RepeatMasker')
    #now soft-mask the genome for gene predictors
    lib.log.info("Soft-masking: running RepeatMasker using %s species" % species)
    if not os.path.isdir(outdir):
        os.makedirs(outdir)
    with open(debug, 'a') as debug_log:
...
def main(args):
    ...
    parser.add_argument('-m', '--method', default='tantan', choices=['repeatmodeler', 'repeatmasker', 'tantan'], help='Method to mask repeats with')
    ...
    if args.method == 'tantan':
        ...
        if not args.repeatmodeler_lib: #no fasta file given, so
            if not args.repeatmasker_species: #no species given, so run entire repeatmodler + repeat masker
                ...
                RepeatModelMask(args.input, args.cpus, tmpdir, args.out, repeats, log_name)
            else:
                RepeatMaskSpecies(args.input, args.repeatmasker_species, args.cpus, tmpdir, args.out, log_name)
        else:
            if lib.checkannotations(args.repeatmodeler_lib):
                RepeatMask(args.input, args.repeatmodeler_lib, args.cpus, tmpdir, args.out, log_name)
            else:
