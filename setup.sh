...
if [ "$dep" = 'pass' ]; then
    ...
    check='pass'
    for i in {python, 'perl' blastp,hmmsearch,hmmscan,augustus,bam2hints,'gmes_petap.pl',mummer,nucmer,show-coords,exonerate,gmap,blat,RepeatModeler,RepeatMasker,pslCDnaFilter,bedtools,bamtools,'gag.py',tbl2asn,'braker.pl',funannotate,mafft,trimal,raxmlHPC-PTHREADS,}; do
        var=$(command -v $i)
        if [ "$var" ]; then
